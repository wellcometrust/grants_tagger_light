...
from grants_tagger_light.evaluation.evaluate_human import evaluate_human


def test_evaluate_human():
    data = [
        {"text": "Lorem", "tags": ["T1", "T2"], "meta": {"Tagger1_tags": ["T1"]}},
        {"text": "Ipsum", "tags": ["T2"], "meta": {"Tagger1_tags": ["T1"]}},
    ]
...
def test_evaluate_human_zero_f1():
    data = [
        {"text": "Lorem", "tags": ["T2"], "meta": {"Tagger1_tags": ["T1"]}},
        {"text": "Ipsum", "tags": ["T2"], "meta": {"Tagger1_tags": ["T1"]}},
    ]
    with tempfile.TemporaryDirectory() as tmp_dir:
        data_path = os.path.join(tmp_dir, "data.jsonl")
...
def test_evaluate_human_perfect_f1():
    data = [
        {"text": "Lorem", "tags": ["T1", "T2"], "meta": {"Tagger1_tags": ["T1", "T2"]}},
        {"text": "Ipsum", "tags": ["T2"], "meta": {"Tagger1_tags": ["T2"]}},
    ]
    with tempfile.TemporaryDirectory() as tmp_dir:
        data_path = os.path.join(tmp_dir, "data.jsonl")
        label_binarizer_path = os.path.join(tmp_dir, "label_binarizer_path")
