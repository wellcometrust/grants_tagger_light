...
from grants_tagger_light.tune_threshold import (
    optimise_threshold,
    confusion_matrix,
...
def test_ones_Y():
    Y_test = np.ones((2, 3))
    Y_pred_proba = np.array([[0.3, 0.5, 0.7], [0.1, 0.8, 0.2]])
...
def test_realistic_Y():
    Y_test = np.array([[0, 0, 1], [1, 0, 0], [0, 1, 0]])
    Y_pred_proba = np.array([[0.3, 0.5, 0.7], [0.6, 0.7, 0.3], [0.1, 0.8, 0.2]])
    optimal_thresholds = optimise_threshold(Y_test, Y_pred_proba)
