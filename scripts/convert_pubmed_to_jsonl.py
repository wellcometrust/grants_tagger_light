...
def main(data_path: str, output_path: str):
    """
    Converts a JSON file containing PubMed abstracts and MeSH terms to a JSONL file
    with a specific format. The PubMed publications come from the Wellcome store.

    The input JSON file should be a list of dictionaries, where each dictionary
    represents a PubMed abstract and has the following keys:
    - "title": a string representing the title of the abstract
    - "abstract": a string representing the text of the abstract
    - "mesh_terms": a list of strings representing the MeSH terms associated with
    the abstract

    The output JSONL file will have one JSON object per line, where each object
    represents a PubMed abstract and has the following keys:
    - "text": a string representing the text of the abstract
    - "tags": a list of strings representing the MeSH terms associated with the
    abstract
